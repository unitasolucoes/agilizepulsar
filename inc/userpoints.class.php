...
class PluginAgilizepulsarUserPoints extends CommonDBTM {
    ...
    public static function removePoints($users_id, $action_type, $reference_id = 0) {
        ...
        $userPoints->fields['points_total'] = max(0, $userPoints->fields['points_total'] - $config);
        $userPoints->fields['points_month'] = max(0, $userPoints->fields['points_month'] - $config);
        $userPoints->fields['points_year'] = max(0, $userPoints->fields['points_year'] - $config);
        $userPoints->fields['date_mod'] = $_SESSION['glpi_currenttime'];
