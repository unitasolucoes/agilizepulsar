...
if ($has_liked) {
    // DESCURTIR
    $DB->delete('glpi_plugin_agilizepulsar_likes', [
        'tickets_id' => $ticket_id,
        'users_id'   => $user_id
    ]);
    ...
    if ($ticket->getFromDB($ticket_id)) {
        ...
        if ($recipient_id > 0) {
            PluginAgilizepulsarUserPoints::removePoints($recipient_id, 'like', $ticket_id);
        }
    }
    ...
    // CURTIR
    $DB->insert('glpi_plugin_agilizepulsar_likes', [
        'tickets_id'    => $ticket_id,
        'users_id'      => $user_id,
    ...
    if ($ticket->getFromDB($ticket_id)) {
        $recipient_id = (int)($ticket->fields['users_id_recipient'] ?? 0);
        if ($recipient_id > 0) {
            PluginAgilizepulsarUserPoints::addPoints($recipient_id, 'like', $ticket_id, true);
        }
    }
